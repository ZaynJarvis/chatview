...
- L0 research uses `codex --search exec` with `codex_l0_reports.schema.json`. These reports now act as the investment-action layer: the primary output is `targets[]`, sorted by `buy_score`, with direct action first, then reason/risk/analysis. The UI renders these as expandable target cards.
- L0 defaults to one consolidated hourly stock-analysis brief (`L0_MAX_REPORTS=1`). The report must prioritize named tickers, plain-Chinese action summaries, short/mid-term moves where useful, and compact risks; sector deep research is only a follow-up section.
