  ...
  return `
    <section class="column l1 ${state.activeLayer === 'L1' ? 'active-layer' : ''}">
      <div class="column-head">
        ...
        <h1>Pending aggregation</h1>
        <p>Scoped to ${escapeHtml(channel?.channel || 'selected channel')}</p>
      </div>
      <div class="toolbar">
        <button disabled>All kinds</button>
        <button disabled>Re-cluster</button>
        <span class="toolbar-note">backend contract pending</span>
      </div>
      <div class="column-body">
        ${detailMarkup()}
        <div class="topic-placeholder">
          <span>Next contract</span>
          <h3>L1 topic clusters</h3>
          <p>This column keeps the design surface ready for clustered topics, citations, and L1 to L2 trace behavior. It is intentionally not deriving insights from raw messages in the browser.</p>
        </div>
      </div>
    </section>
  `;
  ...
  return `
    <section class="column l0 ${state.activeLayer === 'L0' ? 'active-layer' : ''}">
      <div class="column-head">
        ...
        <p>Executive summary, findings, and citations will land here.</p>
      </div>
      <div class="toolbar">
        <button disabled>Regenerate</button>
        <button disabled>Export</button>
        <span class="toolbar-note">not integrated yet</span>
      </div>
      <div class="column-body">
        <article class="report-shell">
          <div class="report-eyebrow">Deep research · L0</div>
          <h2>L2 is live; L0 waits for report API</h2>
          <p class="exec">This pass wires the viewer to the cleaned message API only. The report column preserves the intended reading experience without inventing analysis client-side.</p>
          <h3>Expected report shape</h3>
          <div class="finding"><b>01</b><span>Executive summary with linked citations back to L1 topics and L2 source messages.</span></div>
          <div class="finding"><b>02</b><span>Structured findings ordered by signal strength and confidence.</span></div>
          <div class="finding"><b>03</b><span>Recommendations with source coverage metadata.</span></div>
        </article>
      </div>
    </section>
  `;
